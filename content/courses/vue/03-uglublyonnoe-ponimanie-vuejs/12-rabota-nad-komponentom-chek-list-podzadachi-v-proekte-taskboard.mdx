---
title: "Работа над компонентом чек-лист (подзадачи) в проекте TaskBoard"
type: demo
---

# Шаг 1. Создание компонента чек-листа --- подзадач

Создайте компонент `TaskCardViewTicksList.vue` в директории `src/modules/tasks/components`.

Базовая разметка компонента:
...
# Шаг 2. Создание кнопки добавления новой подзадачи

Добавим кнопку для создания новой подзадачи:
...
Свойство `disabled` переводит компонент в режим «только для чтения», запрещая добавление новых подзадач:
...
const emits = defineEmits(['createTick'])
</script>
```

# Шаг 3. Отображение списка подзадач

Разметка списка подзадач:
...
# Шаг 4. Подключение компонента к TaskView

Интегрируем созданный компонент в `TaskView`:
