---
title: "Добавление обработчиков задач и фильтров в корневой компонент"
type: demo
---

# Шаг 1. Добавление обработчиков задач в App.vue

Теперь добавим в корневой компонент массив задач и обработчики для их создания, изменения и удаления. Временно данные будут храниться здесь, пока мы не перенесём их в глобальное хранилище.
...
# Шаг 2. Добавление обработчиков фильтров в App.vue

Добавим состояние и обработчики для фильтрации:
...
Обратите внимание: отфильтрованные задачи возвращаются через вычисляемое свойство `filteredTasks`, что обеспечивает автоматическое обновление при изменении фильтров.
